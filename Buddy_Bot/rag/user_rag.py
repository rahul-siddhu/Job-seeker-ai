...
class NLUProcessor:
    ...
    async def parse_query(query: str) -> ParsedJobCriteria:
        """Parse natural language query using OpenAI function calling."""
        system_prompt = """
        You are a job search query parser. Extract structured information from the user's query.
        For skills, extract both explicit skills and implied skills from job titles.
        For location, consider cities, states, and regions.
        Examples:
        - "python jobs in karnataka" → skills: ["python"], location: "karnataka"
        - "software engineer bangalore" → skills: ["software engineering"], city: "bangalore"
        """
